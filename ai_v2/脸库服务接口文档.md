...
*   **Response:** (`application/json`)
    *   **`200 OK` - 添加成功:**
        ...
        *   **Example:** (同上一个接口)
    *   **`400 Bad Request` - 图片处理错误:**
        *   同上一个接口 (例如，"图片中没用人脸", "图片中有多张人脸", "人脸分辨率不够")。
        *   响应体: `{"detail": "[ValueError 的错误信息]"}`
    *   **`404 Not Found` - 学生不存在:**
        *   数据库中未找到具有指定 `extra_id` 的学生。
        *   响应体: `{"detail": "Student with extra_id '[提供的 extra_id]' not found"}`
    *   **`500 Internal Server Error` - 处理失败:**
        *   同上一个接口，可能发生在数据库查找、图片保存、数据库写入等步骤。
        *   响应体: `{"detail": "Failed to add face record to database"}` 或 `{"detail": "Internal server error: [具体异常信息]"}`
