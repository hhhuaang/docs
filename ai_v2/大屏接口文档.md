...
- **演示页面地址**: [https://gs.classtao.cn/web/remote.html?school_id=677e4cf2d711833b9a143096](https://gs.classtao.cn/web/remote.html?school_id=677e4cf2d711833b9a143096)
...
- **处理示例**:
  ```javascript
  function handleScreenNavigateUrl(data) {
    const { schoolId, displayId, display_hardware_id, url } = data;
    
    // 校验URL安全性（可选）
    if (!isValidUrl(url)) {
      console.error("无效或不安全的URL");
      return;
    }
    
    // 显示加载提示（可选）
    showMessage(`正在导航到: ${url}`);
...
- **处理示例**:
  ```javascript
  function handleScreenPlayVideo(data) {
    const { schoolId, displayId, display_hardware_id, url, loop } = data;
    
    // 校验URL安全性（可选）
    if (!isValidUrl(url)) {
      console.error("无效或不安全的视频URL");
      return;
    }
    
    // 显示加载提示（可选）
    showMessage(`正在加载视频: ${url}`);
    ...
    videoPlayer.play().catch(error => {
      ...
      showMessage("视频播放失败，可能需要用户交互才能自动播放");
    });
  }
...
- **处理示例**:
  ```javascript
  function handleScreenPlayAudio(data) {
    const { schoolId, displayId, display_hardware_id, url } = data;
    
    // 校验URL安全性（可选）
    if (!isValidUrl(url)) {
      console.error("无效或不安全的音频URL");
      return;
    }
    
    // 显示加载提示（可选）
    showMessage(`正在播放音频: ${url}`);
    ...
    audioPlayer.play().catch(error => {
      ...
      showMessage("音频播放失败，可能需要用户交互才能自动播放");
    });
    
    // 显示音频控制界面（可选）
    showAudioControls(url);
  }
  
  function showAudioControls(url) {
    // 创建或获取音频控制界面
    ...
    if (!audioControls) {
      ...
      audioControls.innerHTML = `
        ...
        <div class="audio-buttons">
          ...
          <button onclick="document.getElementById('audioPlayer').currentTime = 0">重播</button>
      ...
      audioControls.querySelector('.audio-url').textContent = url;
